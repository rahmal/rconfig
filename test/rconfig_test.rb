...
class RConfigTest < Test::Unit::TestCase
  ...
  def setup
    super
    begin
      RConfig._verbose = nil # default
      RConfig.reload(true)
      RConfig._reload_disabled = nil # default
      RConfig._reload_delay = nil # default
    rescue => err
      # NOTHING
    end
  end
  ...
  def test_disable_reload
    ...
    # Reload delay
    RConfig._reload_delay = -1
    # RConfig._verbose = true
    RConfig._config_file_loaded = nil

    # Get the name of a config file to touch.
    assert cf1 = RConfig._get_config_files("test")
    assert cf1 = cf1[0][2]
    ...
    RConfig.disable_reload do 
      ...
      assert ! RConfig._config_file_loaded
    ...
    RConfig._reload_delay = false
    RConfig._verbose = false
  end
  ...
  def test_config_files
    assert_kind_of Array, cf = RConfig._get_config_files("test").select{|x| x[3]}
  ...
  def test_config_changed
    ...
    cf1 = RConfig._config_files("test")
    cf2 = RConfig._get_config_files("test")
    cf3 = RConfig._config_files("test")
    ...
    # STDERR.puts "cf2 = #{cf2.inspect}"
    assert cf1.object_id != cf3.object_id
    assert RConfig._config_files("test")
    assert_equal false, RConfig.config_changed?("test")

    # Pull config again, expect no changes.
    cf4 = RConfig._config_files("test")
    # STDERR.puts "cf3 = #{cf1.inspect}"
    # STDERR.puts "cf2 = #{cf2.inspect}"
  ...
  def test_check_reload_disabled
    ...
    assert_kind_of Array, RConfig._config_files('test')
    
    RConfig._reload_disabled = true

    assert_kind_of Array, RConfig.load_config_files('test')

    RConfig._reload_disabled = nil
  end
  ...
  def test_overlay_by_name
    assert_equal nil,   RConfig._overlay
